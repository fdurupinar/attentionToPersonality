...
//|| animation[_animInfo.AnimName].normalizedTime > 1f) {   //Call in update to prevent it from calling lateupdate
    ...
    private void BendArms() {
        ...
        /*  Vector3 elbowDirR = Vector3.Cross(
              _targetR - _shoulderRTargetPos,
              Vector3.Cross(
                  _targetR - _shoulderRTargetPos,
                  _targetR - _elbowRTargetPos)
              );
          */
        Vector3 elbowDirR = Vector3.Cross(
        ...
        /*    Vector3 elbowDirL = Vector3.Cross(
                _targetL - _shoulderLTargetPos,
                Vector3.Cross(
                    _targetL - _shoulderLTargetPos,
                    _targetL - _elbowLTargetPos)
                );
            */
        Vector3 elbowDirL = Vector3.Cross(
...
//enclosing
                //HACK: Mia has wider hips           
                //if (_animInfo.CharacterName.Contains("Mia") || _animInfo.CharacterName.Contains("mia"))
                //    encSprOffset = 0.02f * Mathf.Lerp(EncSpr[0], EncSpr[1], shapeT);
                // else
...
//enclosing
                //HACK: Mia has wider hips           
                //  if (_animInfo.CharacterName.Contains("MIA"))
                //     encSprOffset = 0.02f * Mathf.Lerp(EncSpr[1], EncSpr[2], shapeT);
                // else
        ...
        //FUNDA: Prevents incorrect arm swivels in pointing
       ...
       // _fbIk.solver.rightShoulderEffector.rotationWeight = 0f;
        ...
        //FUNDA WHY??
    /*    if (_animInfo.AnimName.ToUpper().Contains("WALK") && encSprInit > 0) {
            encSprInit = 0;
            encSprAngle = 0f;
        }*/
        //Feet
