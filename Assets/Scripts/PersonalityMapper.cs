    ...
    public void MapAnimSpeeds(AnimationInfo[] agentScripts, float minDesiredSpeed, float maxDesiredSpeed) {
          ...
          //      return;
            ...
            foreach (AnimationInfo t in agentScripts) {
                ...
                if (max == min) { //means only one agent
                    if (t.AnimSpeed < minDesiredSpeed)
                        t.AnimSpeed = minDesiredSpeed;
                    else if (t.AnimSpeed > maxDesiredSpeed)
                        t.AnimSpeed = maxDesiredSpeed;
                }
                else
